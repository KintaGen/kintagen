...
access(all) contract PublicKintaGenNFTv6: NonFungibleToken {
    ...
    access(self) var nftOwners: {UInt64: Address}
    // --- Events ---
    ...
    access(all) resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver {
        ...
        init(projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String) {
            self.id = PublicKintaGenNFTv6.totalSupply
        ...
        access(all) view fun getViews(): [Type] {
            return [
                ...
                Type<[PublicKintaGenNFTv6.WorkflowStepView]>()
            ]
        }
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return PublicKintaGenNFTv6.resolveContractView(
                        resourceType: nil,
                ...
                case Type<[PublicKintaGenNFTv6.WorkflowStepView]>():
        ...
        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- PublicKintaGenNFTv6.createEmptyCollection(nftType: Type<@PublicKintaGenNFTv6.NFT>())
        }
    }
    ...
    access(all) resource Collection: NonFungibleToken.Collection, ViewResolver.ResolverCollection {
        ...
        access(all) fun deposit(token: @{NonFungibleToken.NFT}) {
            let nft <- token as! @PublicKintaGenNFTv6.NFT
            let id = nft.id
            PublicKintaGenNFTv6.nftOwners[id] = self.owner!.address
        ...
        access(all) view fun getSupportedNFTTypes(): {Type: Bool} {
            return { Type<@PublicKintaGenNFTv6.NFT>(): true }
        }

        access(all) view fun isSupportedNFTType(type: Type): Bool {
            return type == Type<@PublicKintaGenNFTv6.NFT>()
        }
    ...
    access(all) fun mint(recipientAddress: Address,projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String): @NFT {
        ...
        PublicKintaGenNFTv6.totalSupply = PublicKintaGenNFTv6.totalSupply + 1
        self.nftOwners[token.id] = recipientAddress
        return <- token
    }
    access(all) view fun getOwner(id: UInt64): Address? {
        return self.nftOwners[id]
    }
        ...
        switch viewType {
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(
                    ...
                    publicCollection: Type<&PublicKintaGenNFTv6.Collection>(),
                    publicLinkedType: Type<&PublicKintaGenNFTv6.Collection>(),
    ...
    init() {
        self.CollectionStoragePath = /storage/kintagenV6Collection
        self.CollectionPublicPath = /public/kintagenV6Collection
        self.totalSupply = 0
        self.nftOwners = {}
        ...
        if !self.account.capabilities.exists(self.CollectionPublicPath) {
            self.account.capabilities.publish(
                self.account.capabilities.storage.issue<&PublicKintaGenNFTv6.Collection>(self.CollectionStoragePath),
