...
access(all) contract PublicKintaGenNFTv4: NonFungibleToken {

    access(all) let CollectionStoragePath: StoragePath
    access(all) let CollectionPublicPath: PublicPath
    access(all) let MinterStoragePath: StoragePath
    access(all) let MinterPublicPath: PublicPath // Path for the public minter capability
    ...
    access(all) resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver {
        ...
        init(projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String) {
            self.id = PublicKintaGenNFTv4.totalSupply
        ...
        access(all) view fun getViews(): [Type] {
            return [
                ...
                Type<[PublicKintaGenNFTv4.WorkflowStepView]>()
            ]
        }
        ...
        access(all) fun resolveView(_ view: Type): AnyStruct? {
            switch view {
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return PublicKintaGenNFTv4.resolveContractView(
                        resourceType: nil,
                ...
                case Type<[PublicKintaGenNFTv4.WorkflowStepView]>():
        ...
        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- PublicKintaGenNFTv4.createEmptyCollection(nftType: Type<@PublicKintaGenNFTv4.NFT>())
        }
    }
    ...
    access(all) resource Collection: NonFungibleToken.Collection, ViewResolver.ResolverCollection {
        ...
        access(all) fun deposit(token: @{NonFungibleToken.NFT}) {
            let nft <- token as! @PublicKintaGenNFTv4.NFT
            let id = nft.id
        ...
        access(all) view fun getSupportedNFTTypes(): {Type: Bool} {
            return { Type<@PublicKintaGenNFTv4.NFT>(): true }
        }

        access(all) view fun isSupportedNFTType(type: Type): Bool {
            return type == Type<@PublicKintaGenNFTv4.NFT>()
        }
    ...
    access(all) resource Minter {
        access(all) fun mint(projectName: String, projectSummary: String, projectCID: String, principalInvestigator: String, runHash: String): @NFT {
            ...
            PublicKintaGenNFTv4.totalSupply = PublicKintaGenNFTv4.totalSupply + 1
            return <- token
        }
    }
    ...
    // This function is kept for the contract owner's access
    access(all) fun borrowMinter(): &PublicKintaGenNFTv4.Minter {
        return self.account.storage.borrow<&PublicKintaGenNFTv4.Minter>(from: self.MinterStoragePath)
            ?? panic("PublicKintaGenNFTv4 minter is not stored in contract account.")
    }

    // New public function to allow anyone to borrow the minter
    access(all) fun borrowPublicMinter(): &PublicKintaGenNFTv4.Minter {
        let cap = self.account.capabilities.get<&PublicKintaGenNFTv4.Minter>(self.MinterPublicPath)
        return cap.borrow() ?? panic("Could not borrow public minter capability")
    }
        ...
        switch viewType {
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(
                    ...
                    publicCollection: Type<&PublicKintaGenNFTv4.Collection>(),
                    publicLinkedType: Type<&PublicKintaGenNFTv4.Collection>(),
    ...
    init() {
        self.CollectionStoragePath = /storage/kintagenV4Collection
        self.CollectionPublicPath = /public/kintagenV4Collection
        self.MinterStoragePath = /storage/kintagenV4Minter
        self.MinterPublicPath = /public/kintagenV4Minter // Initialize the public path
        self.totalSupply = 0
        ...
        let minterCap = self.account.capabilities.storage.issue<&PublicKintaGenNFTv4.Minter>(self.MinterStoragePath)
        self.account.capabilities.publish(minterCap, at: self.MinterPublicPath)

        // The following lines for the collection remain as they were
        if self.account.storage.borrow<&Collection>(from: self.CollectionStoragePath) == nil {
            self.account.storage.save(<- create Collection(), to: self.CollectionStoragePath)
        }
        
        let cap = self.account.capabilities.storage.issue<&PublicKintaGenNFTv4.Collection>(self.CollectionStoragePath)
